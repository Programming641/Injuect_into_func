...
# in the command line parameters, after python file to be executed comes java top directory
rootDir = args[1]
...
# for looping all commandline parameters
# looking for string, "delete FUNCTION_TRACING"
arg_counter = 0
DELETE_FUNC = False
...
for arg in sys.argv:
    ...
    if "delete" in arg:
        # if current parameter is delete, look for FUNCTION_TRACING string in the next parameter
        if "FUNCTION_TRACING" in sys.argv[arg_counter + 1 ]:
            DELETE_FUNC = True
...
for dirs, subdirs, files in os.walk(rootDir):
    ...
    for file in files:
        if ".java" in file:
            ...
            # delete FUNCTION_TRACING from java file
            if DELETE_FUNC:

                while file_line:
                    
                    if "FUNCTION_TRACING" in file_line:
                        # current line contains FUNCTION_TRACING so do not add this line to new file. just read next line
                        file_line = file_read.readline()


                    else:
                        # current line is not FUNCTION_TRACING. add this line to new file and read next line
